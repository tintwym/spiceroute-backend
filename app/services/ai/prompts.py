...
from app.models.cuisine_catalog import ai_cuisine_enum_values
...
RECIPE_RESPONSE_SCHEMA: dict = {
    ...
    "properties": {
        ...
        "cuisine": {
            ...
            "enum": ai_cuisine_enum_values(),
        },
        "language": {
            "type": "string",
            # MUST stay in lock-step with `SUPPORTED_LANGUAGES` in
            # `app/schemas/spice_route.py`. Drift between the two has
            # historically caused silent generation bugs — the model
            # emits a language code that the validator then rejects
            # with a 422, with no obvious link back to "the prompt's
            # enum says yes but the schema says no". Update both.
            "enum": ["en", "zh", "ja", "ko", "vi"],
        },
...
def chat_system_prompt(language: str) -> str:
    ...
    return (
        ...
        "You help home cooks across seventy-seven cuisines spanning "
        "East Asia (Korean, Japanese, Chinese, Taiwanese, Mongolian, "
        "Tibetan, Hong Kong, Macanese, Sichuan, Cantonese, Shanghainese, "
        "Fujian, Hunan, Yunnan, Beijing, Northeast Chinese, Hakka, "
        "Uyghur, Okinawan, Shandong, Guangxi, Teochew, Hainanese, "
        "Jiangsu, Zhejiang, Anhui, Jiangxi, Guizhou, Manchurian, Shaanxi), "
        "Southeast Asia (Burmese, Cambodian, Thai, Vietnamese, "
        "Filipino, Indonesian, Malaysian), "
        "South Asia (Indian, Pakistani, Sri Lankan), "
        "Europe (Italian, French, Greek, Spanish, German, Portuguese, "
        "British), "
        "the Americas (American/Western, Mexican, Brazilian, Peruvian, "
        "Caribbean), and "
        "the Middle East & Africa (Lebanese, Turkish, Moroccan, "
        "Ethiopian). "
