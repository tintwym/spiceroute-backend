...
# (spiceroute-flutter/lib/l10n/*.arb). Adding a language means
# updating both this tuple and the matching ARB file. Removing one
# means dropping the ARB and pruning here so the API rejects the
# now-unsupported value (otherwise old clients could keep posting
# orphaned `language` strings).
#
# History: the product swapped Thai (`th`) for Burmese (`my`) in v2,
# and later removed Burmese entirely. Old `th` rows were migrated in
# `alembic/versions/0002_savor_global.py`; old `my` rows are left
# in place — they're harmless without a UI that renders them, and
# re-introducing the locale later just means re-adding `"my"` here.
SUPPORTED_LANGUAGES = ("en", "zh", "ja", "ko", "vi")
...
class SpiceRouteBase(BaseModel):
    ...
    # Optional on input — the write path auto-computes a value via
    # `compute_difficulty()` when the client doesn't pin one, so a
    # blank submission still lands a row with a non-null difficulty.
...
class SpiceRouteSummary(BaseModel):
    ...
    # Non-optional in the response — every row in the DB now carries a
    # validated value (NOT NULL with server default). Clients can rely on
    # the field always being present, which removes the "hide the chip
    # when missing" UI branch.
