...
class Settings(BaseSettings):
    ...
    # Per-IP rate limits on the recipe CRUD surface. These complement
    # the AI quotas above by capping abuse of the cheap-per-request
    # but easy-to-spam read endpoints (anyone can scrape the curated
    # catalog otherwise) and the authenticated write endpoint (caps
    # a single user-controlled IP from publishing thousands of
    # spam recipes a day even after they get a token).
    #
    # GETs are hourly because the alternative (daily) lets an attacker
    # burst the entire budget in seconds and then go silent. POST is
    # daily because legitimate use is bursty (publishing 5 recipes
    # in one sitting is normal; 50 a day is not).
    #
    # Defaults sized for "normal browsing pattern": ~300 list calls /
    # hour is roughly one new page every 12 seconds for the whole
    # hour — well above any human browser, well below a scraper's
    # tens of calls per second.
    recipe_list_per_hour: int = 300
    recipe_detail_per_hour: int = 600
    recipe_writes_per_day: int = 50
