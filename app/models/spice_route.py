...
class SpiceRoute(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    #     "vi": {"title": "Quiche kiểu Lorraine",   "description": "..."},
    #     ...
    #   }
    #
    ...
    # How challenging the recipe is to execute. NOT NULL with a server-side
    # default of MEDIUM so the column is uniformly populated across the
    # whole table (the audit revealed many rows were missing this in the
    # client-derived form, where the chip was hidden on narrow viewports
    # and computed from a flawed `totalMinutes + steps*5` heuristic).
    # The default exists only as a safety net for INSERTs that forget
    # the field; the API write path auto-computes via
    # `Difficulty.compute_difficulty()` so well-behaved callers never
    # hit the default.
