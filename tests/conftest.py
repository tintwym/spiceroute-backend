...
async def client(
    ...
    # we run on sqlite, so swap the checks for no-ops. Tests that WANT to
    # exercise throttling override these back per-test via `monkeypatch`
    # (see `test_recipe_throttle.py`).
    async def _noop(*_args, **_kwargs) -> None:
        return None

    monkeypatch.setattr(_rate_limit, "check_recipe_quota", _noop)
    monkeypatch.setattr(_rate_limit, "check_chat_quota", _noop)
    monkeypatch.setattr(_rate_limit, "check_recipe_list_quota", _noop)
    monkeypatch.setattr(_rate_limit, "check_recipe_detail_quota", _noop)
    monkeypatch.setattr(_rate_limit, "check_recipe_write_quota", _noop)
