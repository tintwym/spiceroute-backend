...
@pytest.mark.asyncio
async def test_chinese_filter_includes_provincial_rows(client: AsyncClient) -> None:
    headers = auth_header(uid="alice")
    for spec in [
        {"title": "Mapo Tofu", "cuisine": "sichuan"},
        {"title": "Korean Bowl", "cuisine": "korean"},
    ]:
        await client.post(
            "/spice_routes", json=_recipe_payload(**spec), headers=headers
    ...
    r = await client.get("/spice_routes", params={"cuisine": "chinese"})
    assert r.status_code == 200
    titles = [i["title"] for i in r.json()["items"]]
    assert titles == ["Mapo Tofu"]


@pytest.mark.asyncio
async def test_burmese_filter_includes_regional_rows(client: AsyncClient) -> None:
    headers = auth_header(uid="alice")
    for spec in [
        {"title": "Tea Shop Eggs", "cuisine": "yangon"},
        {"title": "Korean Bowl", "cuisine": "korean"},
    ]:
        await client.post(
            "/spice_routes", json=_recipe_payload(**spec), headers=headers
    ...
    r = await client.get("/spice_routes", params={"cuisine": "burmese"})
    assert r.status_code == 200
    titles = [i["title"] for i in r.json()["items"]]
    assert titles == ["Tea Shop Eggs"]
...
async def test_translate_to_swaps_title_and_description(
    ...
    Originally written when the bug was "Burmese UI shows English
    Quiche Lorraine title + description". Burmese is no longer a UI
    locale, but the regression guard still pins the behavior the
    curated translations rely on:
      * `translate_to=ko` (full override)        → both swap.
      * `translate_to=<desc-only locale>`        → title falls back to
        source, description swaps. Uses `ja` (a supported UI locale).
    """
    ...
    row = SpiceRoute(
        ...
        translations={
            ...
            # Description-only entry (no "title" key) — the fallback
            # path should keep the original "Quiche Lorraine" but
            # swap in this description. This is the exact contract
            # the curated seed relies on for any dish/locale combo
            # whose name has no settled native transliteration.
            "ja": {
                "description": "バター風味のショートクラストにベーコンとクリーム。",
            },
        },
    )
    db_session.add(row)
    ...
    # Description-only entry: title falls back, description swaps.
    r = await client.get("/spice_routes", params={"translate_to": "ja"})
    item = r.json()["items"][0]
    assert item["title"] == "Quiche Lorraine"
    assert item["description"] == "バター風味のショートクラストにベーコンとクリーム。"
...
@pytest.mark.asyncio
async def test_translate_to_rejects_garbage_locale_strings(
    client: AsyncClient, db_session
) -> None:
    """LLM garbage in `translations[locale]` must fall back to source copy."""
    from app.models.spice_route import SpiceRoute
    ...
    db_session.add(
        SpiceRoute(
            ...
            translations={
                "zh": {
                    "title": "红尺索尺零和索尺索尺",
                    "description": "常于的索尺零和尺索尺中常尺索尺一个尺索尺一个尺索尺",
                }
            },
        )
    )
    ...
    r = await client.get("/spice_routes", params={"translate_to": "zh"})
    assert r.status_code == 200, r.text
    item = next(
        i for i in r.json()["items"] if i["title"] == "Dezhou Braised Chicken"
    )
    assert item["title"] == "Dezhou Braised Chicken"
    assert item["description"] == "Famous braised chicken with aromatic spices."


@pytest.mark.asyncio
async def test_list_hides_retired_eastern_european_rows(
    client: AsyncClient, db_session
) -> None:
    """Legacy `eastern_european` rows must not 500 the public list."""
    ...
    db_session.add(
        SpiceRoute(
            ...
            cuisine=Cuisine.EASTERN_EUROPEAN,
            language="en",
            prep_minutes=10,
            cook_minutes=30,
            servings=4,
    ...
    r = await client.get(
        "/spice_routes",
        params={"translate_to": "en", "limit": 30, "offset": 0},
    )
    assert r.status_code == 200, r.text
    titles = [item["title"] for item in r.json()["items"]]
    assert "Borscht" not in titles
    assert "Pad Thai" in titles
